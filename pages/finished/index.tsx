...
const Index = () => {
  ...
  const { finishedTodos, searchState, searchedFinished } = useSelector(
  ...
  const onChangePageCountNum = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setNewFinishedTodos(finishedTodos.filter((todo, index) => index < parseInt(e.target.value)));
  };
          ...
          <div className="listTodo-wrapper">
            <ul>
              <li className="listTodo-title">
                ...
                {!searchState && <div className="todo-finished">Move to Todo list</div>}
              </li>
              {newFinishedTodos &&
              ...
              {searchedFinished &&
                searchedFinished.length > 0 &&
                searchState &&
                searchedFinished.map((todo) => (
                  <li key={todo.id} className="listTodo-contents">
                    <div className="todo-name">{todo.todo}</div>
                    <div className="todo-importance">
                      {Array.from(Array(todo.importance + 1).keys()).map((arr, index) => (
              ...
              {newFinishedTodos.length === 0 && !searchState && (
                <div className="nodata">
                  404 ...? No, <br />
                  There is no data that I have done yet...
                </div>
              )}
              {searchedFinished.length === 0 && searchState && (
                <div className="nodata">
